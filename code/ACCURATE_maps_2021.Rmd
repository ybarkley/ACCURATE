...
swdata = read.csv("C:\\Users\\yvers\\OneDrive\\PHD\\Chapter 2-Sperm Whale Loc\\data\\Sperm_Whale_Acoustic_Locations_EEZonly_20190422.csv")

swdata = filter(swdata, swdata$Cruise.Number > 1203)

swdf0 <- swdata[,]
swdf0 <- swdata[, c(2,4:13)]
...
swdf0$sighted <- NA
swdf0$sighted <- ifelse(swdf0$visual_id != 999, "1", "0")   #for encounters with no visual
swdf0$sighted <- ifelse(swdf0$acoustic_id == 999, "2", swdf0$sighted)   #for encounters with no visual


#modified by YB on Aug 12, 2019(?or 2020...)
## make a new column with text describing sighted, unsighted
swdf0$sighted2[swdf0$sighted =="0"]<-"Acoustics Only (n=242)"
swdf0$sighted2[swdf0$sighted =="1"]<-"Acoustics and Visuals (n=72)"
swdf0$sighted2[swdf0$sighted =="2"]<-"Visuals Only (n=13)"
...
# Some Data Checks #### 
 # YB 9/2021
swdf0 %>%
   group_by(sighted, Cruise.Number) %>% tally()
...
us<-readShapePoly("C:\\Users\\yvers\\OneDrive\\PHD\\Chapter 2-Sperm Whale Loc\\code\\yb_maps\\shp\\tl_2016_us_state.shp")
...
ggplot() +  ## this just makes the background white
  ...
  scale_color_manual(values = c('darkorange1','forestgreen','darkorchid3'))+  #c('darkred','navy','darkviolet')
  
  ## add the map downloaded above
  geom_polygon(data = hawaiiMap, aes(x = long, y = lat, group = group), fill = 'gray30', color = 'gray10') +
  coord_fixed(1) +
  
  ## now add the data points
  geom_point(data = swdf, size = 3,
  aes(
  x = lon2,
  y = lat,
  pch = factor(sighted),
